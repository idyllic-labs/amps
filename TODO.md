...
### Current `mdx-ai agent <dir>` Behavior
...
- `agent-discovery.ts` — scans `~/.mdx-ai/agents/` for agent dirs with `agent.mdx`
...
2. Keep `mdx-ai agent <dir>` as the simple "one spec, one instance" path
3. Design the environment concept separately, maybe as `mdx-ai env` commands
