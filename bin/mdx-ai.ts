...
function printProviders() {
  ...
  process.stdout.write(`\n`);
  process.stdout.write(`  ${c.dim}Change with:${c.reset} mdx-ai providers default ${c.dim}<provider/model>${c.reset}\n\n`);
}

function setDefaultModel(model: string) {
  if (!model.includes("/")) {
    process.stderr.write(`${c.red}Error:${c.reset} Model must be in provider/model format (e.g., azure/gpt-5.2)\n`);
    process.exit(1);
  }
  ...
  if (providerDef) {
    ...
    if (!configured) {
      process.stderr.write(`${c.yellow}Warning:${c.reset} ${providerDef.name} is not configured (missing env vars)\n`);
    }
  }
...
program
  ...
  .action((_args) => {
    ...
    const binPath = new URL("../packages/workflow/bin/mdx-ai-workflow.ts", import.meta.url).pathname;
    spawn(binPath, raw);
  });
