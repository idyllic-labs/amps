...
async function runExecute(args: CLIArgs, filePath: string): Promise<void> {
  ...
  const interactive = args.interactive ??
    (process.stdout.isTTY && !args.stream && args.outputFormat === "pretty");
  ...
  const cliInputResolver: InputResolver = async (request) => {
    ...
    switch (request.kind) {
      case "prompt":
        ...
        error(`Interactive input "${request.name}" requires a value in non-interactive mode. Use --input ${request.name}=value`, 1);
      case "select":
