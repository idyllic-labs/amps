 ...
 */
function parseInputTypeDecl(raw: string): {
  type: InputType;
 ...
 */
function tryParseElse(
  source: string,
  pos: number,
): { children: WorkflowNode[]; end: number } | null {
  let i = pos;
  ...
  function validateNodes(nodes: WorkflowNode[]) {
    for (const node of nodes) {
      switch (node.kind) {
        case "generation":
          if (!node.name)
            errors.push({ message: "Generation node is missing required 'name' prop" });
          break;
        case "structured":
          if (!node.name)
            errors.push({ message: "Structured node is missing required 'name' prop" });
          validateFields(node.fields);
          break;
        case "websearch":
          if (!node.name)
            errors.push({ message: "WebSearch node is missing required 'name' prop" });
          break;
        case "webfetch":
        ...
        case "prompt":
          ...
          if (!node.message.raw)
            errors.push({ message: "Prompt node is missing required 'message' prop" });
          break;
        case "select":
          if (!node.name) errors.push({ message: "Select node is missing required 'name' prop" });
          if (!node.message.raw)
            errors.push({ message: "Select node is missing required 'message' prop" });
          break;
        case "confirm":
          if (!node.name) errors.push({ message: "Confirm node is missing required 'name' prop" });
          if (!node.message.raw)
            errors.push({ message: "Confirm node is missing required 'message' prop" });
          break;
        case "prose":
  ...
  function validateFields(fields: FieldDef[]) {
    for (const field of fields) {
      if (!VALID_FIELD_TYPES.has(field.type)) {
        errors.push({
          message: `Invalid field type: "${field.type}" (use one of: text, number, boolean, list, object)`,
        });
      }
