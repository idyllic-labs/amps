...
class StatusLine implements Component {
  ...
  render(width: number): string[] {
    ...
    const right = this.rightText
      ? `${theme.muted}${this.rightText}${theme.reset}`
      : "";
...
class HeaderBox implements Component {
  ...
  render(width: number): string[] {
    ...
    for (const line of this.inputLines) {
      ...
      lines.push(
        `${theme.primary} │${theme.reset}  ${padded}${theme.primary}│${theme.reset}`,
      );
    }
    if (this.inputLines.length === 0) {
      const empty = " ".repeat(innerWidth);
      lines.push(
        `${theme.primary} │${theme.reset}${empty}${theme.primary}│${theme.reset}`,
      );
    }

    lines.push(
      `${theme.primary} ╰${"─".repeat(innerWidth)}╯${theme.reset}`,
    );
    return lines;
  }
}
...
export class WorkflowTUI {
  ...
  constructor(
    ...
    // Footer
    this.footer = new WorkflowFooter(
      workflow.name,
      this.model,
  ...
  private buildSkeleton(
    ...
    for (const node of nodes) {
      switch (node.kind) {
        ...
        case "if": {
          const conditionLabel = node.condition.raw.length > 30
          ...
          if (node.elseChildren && node.elseChildren.length > 0) {
            parent.addChild(
              new Text(
                " ".repeat(indent) + `${theme.muted}Else${theme.reset}`,
                0,
                0,
              ),
            );
        ...
        case "select":
        case "confirm": {
          const label = node.kind === "prompt" ? nodeLabels.prompt
            : node.kind === "select" ? nodeLabels.select
            : nodeLabels.confirm;
          const sl = new StatusLine(
            icons.input,
            label,
  ...
  private collectChildNames(nodes: WorkflowNode[], names: string[]): void {
    for (const node of nodes) {
      ...
      if (
        "elseChildren" in node &&
        Array.isArray((node as any).elseChildren)
      ) {
  ...
  handleEvent(event: StreamEvent): void {
    switch (event.type) {
      ...
      case "input:end": {
        ...
        const displayValue = typeof event.value === "boolean"
          ? (event.value ? "Yes" : "No")
          : String(event.value);
