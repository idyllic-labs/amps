...
export function parseAgentIdentity(markdown: ParsedMarkdown): AgentIdentity {
  ...
  const purposeSection = markdown.sections.find((s) => s.heading.toLowerCase().includes("purpose"));
  ...
  const capabilities = capabilitiesSection ? extractListItems(capabilitiesSection.content) : [];

  const constraintsSection = markdown.sections.find((s) =>
    s.heading.toLowerCase().includes("constraints"),
  );
  const constraints = constraintsSection ? extractListItems(constraintsSection.content) : [];
...
export function parseHeartbeat(markdown: ParsedMarkdown): HeartbeatConfig {
  ...
  const onWakeSection = markdown.sections.find((s) => s.heading.toLowerCase().includes("on wake"));
  const onWake = onWakeSection ? extractListItems(onWakeSection.content) : [];

  const routineSection = markdown.sections.find((s) => s.heading.toLowerCase().includes("routine"));
  ...
  const contextSection = markdown.sections.find((s) => s.heading.toLowerCase().includes("context"));
