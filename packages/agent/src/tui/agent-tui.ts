...
export class AgentTUI {
  ...
  private async handleSubmit(text: string) {
    ...
    this.chatContainer.addChild(
      new Text(`${theme.userBg}${theme.userText} ${text} ${theme.reset}`, 1, 0)
    );
    ...
    try {
      ...
      this.streamingComponent = new Markdown(this.streamingMessageText, 1, 0, markdownTheme);
      this.chatContainer.addChild(new Spacer(1));
      this.chatContainer.addChild(this.streamingComponent);

      let lastUsage: ChatMessage["usage"] | undefined;
      ...
      for await (const event of this.runtime.processTaskStream(text, filteredHistory)) {
        if (event.type === "text_delta") {
          this.streamingMessageText += event.delta;
      ...
      await this.sessionManager.saveMessage({
        role: "assistant",
        content: this.streamingMessageText,
        timestamp: new Date().toISOString(),
        usage: lastUsage,
      });
  ...
  private async handleCommand(command: string) {
    ...
    switch (cmd) {
      case "/help":
        this.chatContainer.addChild(new Text(`${theme.muted}Commands: /clear /skills /quit${theme.reset}`, 1, 0));
        this.chatContainer.addChild(new Spacer(1));
        this.tui.requestRender();
        break;
      ...
      case "/skills":
        ...
        this.chatContainer.addChild(new Text(`${theme.muted}Skills: ${skills.join(", ")}${theme.reset}`, 1, 0));
        this.chatContainer.addChild(new Spacer(1));
        this.tui.requestRender();
        break;
      ...
      default:
        this.chatContainer.addChild(new Text(`${theme.muted}Unknown: ${command}${theme.reset}`, 1, 0));
        this.chatContainer.addChild(new Spacer(1));
        this.tui.requestRender();
    }
  }
  ...
  private async loadChatHistory() {
    ...
    for (const msg of history) {
      if (msg.role === "user") {
        this.chatContainer.addChild(
          new Text(`${theme.userBg}${theme.userText} ${msg.content} ${theme.reset}`, 1, 0)
        );
        this.chatContainer.addChild(new Spacer(1));
  ...
  async start() {
    ...
    if ((await this.sessionManager.loadHistory()).length === 0) {
      this.chatContainer.addChild(new Text(`${theme.success}Welcome to mdx-ai!${theme.reset}`, 1, 0));
      this.chatContainer.addChild(new Spacer(1));
    }
  ...
  private getFooterStats(): FooterStats {
    return {
      ...
      sessionName: "main",
      workingMessage: this.isProcessing ? `${this.defaultWorkingMessage} (${thinkingIndicator})` : undefined,
